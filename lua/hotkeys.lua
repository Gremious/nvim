...
keymap.set(
	{ modes.NORMAL, modes.VISUAL_AND_SELECT },
	"j",
	"gj",
	{ desc = "Navigate down by visual line instead of text line." }
)
keymap.set(
	{ modes.NORMAL, modes.VISUAL_AND_SELECT },
	"k",
	"gk",
	{ desc = "Navigate up by visual line instead of text line." }
)
keymap.set(
	{ modes.NORMAL, modes.VISUAL_AND_SELECT },
	"<C-s>",
	":update<CR>",
	{ silent = true, desc = "Ctrl+s to save." }
)
keymap.set(
	{ modes.NORMAL },
	"<leader>=",
	"m`va{==````",
	{ desc = "Autoformat around current {} scope" }
)
...
-- you can do ctrl-v=1+1<CR> to insert math
